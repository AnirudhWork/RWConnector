...
export const validateLoginCred = async (
  username: string,
  password: string,
  ...
  try {
    ...
    const response = await new APIServices(false, navigation, dispatch).post(
      API_ENDPOINT.LOGIN,
      body,
    );
    if (response?.status === STATUS_CODES.SUCCESS) {
      printLogs(TAG, '| successful login RESPONSE data:', response.data);
      const success: boolean = await AsyncStorageUtils.setLoginResponse(
        response.data,
      );
      if (success) {
        navigation.reset({
    ...
    if (axios.isCancel(error)) {
      ...
      printLogs(
        TAG,
        '| RESPONSE error with status code 401 while trying to validate user, invalid username/password. ErrorMsg:',
        knownError?.response,
      );
